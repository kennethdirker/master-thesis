...
def process_images(input_obj: dict, context: dict) -> dict:
    ...
    # 2 ways to do merge_nested, 1 way to do merge_flattened:
    # merge_nested function: merge_nested(noiseremover_out["output"], inputs["fit_list"])
    # merge_nested inline: [noiseremover_out["output"], inputs["fit_list"]]
    # merge_flattened: merge_flattened(noiseremover_out["output"], inputs["fit_list"])
	after_plot_inspect_in = {
		"input_fits": merge_flattened(noiseremover_out["output"], inputs["fit_list"]), # merge_flattened
