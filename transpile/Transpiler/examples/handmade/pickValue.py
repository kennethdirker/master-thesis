...
from CWL2DASK.scripting import all_non_null, FileObject, glob, js_eval, load_input_object, merge_flattened, scatterizer, transpose
...
def process_images(input_obj: dict, context: dict) -> dict:
	...
	mockup_out = dask.delayed(transpose)(noiseremover_scattered_out)

	# Step ID:    after_plot_inspect
	# Step label: imageplotter
	after_plot_inspect_in = {
		"input_fits": all_non_null(merge_flattened(
			noiseremover_out["output"], 
			mockup_out["output"], 
			inputs["fit_list"])),
